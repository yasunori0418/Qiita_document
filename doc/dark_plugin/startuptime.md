...
`lightline.vim`の起動タイミングとして、`CursorHold/CursorMoved/InsertEnter/CmdlineEnter`のいずれかのイベントが発生。
<!-- textlint-disable -->
...
ここまで設定できて、私のNeovimでは冒頭で書いたような起動時間になりました。
しかし、この起動時間はあくまでも計測する中で最速のタイムであり、一定して11msで起動できるわけではありません。数値としてもブレが発生するものです。
起動時間が早くなることで、ちょっとした優越感もありますし、Vim活が捗らないときにはちょうど良い作業かと思います。
皆さんもぜひ試してみてください。

あと、気付かれた方もいらっしゃるかと思いますが、この記事dein.vimの解説も兼ねています。
参考になる設定を探すときにGitHubでコード検索を使うのですが、読んでいてうまく動いてなさそうな設定とかも見るようになったので、この記事が参考になれば良いなとも思っています。
dein.vimは本当に高機能なプラグインマネージャーですので、100%に扱いこなすのは難しいのです。
こんなことを書いている張本人でさえ、まだまだ分からないこともありますので、先に設定されている皆さんのコードを参考にさせてください…。
