...
オプションでは主に`lazy`有無で、2つのtomlファイルを用意すると思います。
ここから、すでに高速化は始まっていて、最初の鍵になるのは遅延設定をしていないtomlファイルなのです。

どちらかというと`VimEnter`の後に、プラグインを起動させることが高速化につながるのですが、dein.vimは違います。
遅延設定していないプラグインにこそ、高速化のひと手間を解こしているのです。
先に解説した通り、プラグインは`runtimepath`に指定されたパス内のディレクトリにあるスクリプトを読み込むことで、プラグインとして成り立ちます。

この`runtimepath`というのは指定された分だけちゃんと順番に読み込みますので、数が多くなればなるほど、読み込みに時間がかかるようになるのです。
...
マージして読み込みにいくディレクトリを少くしても、読み込むスクリプトが多ければ読み込みに時間がかかるので、マージ機能を過信しないでください。
あくまで、読み込みを高速化するために、ひと手間処理をしてくれているものと思ってください。


### tomlなどに記述した設定も`state_nvim.vim`[^2]に集結する。
